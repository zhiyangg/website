...
- **`box-sizing: content-box` on `.typed-reveal`** (`Header.astro`) is load-bearing. Tailwind's global `border-box` otherwise lets the 1px typewriter cursor border eat into `width: 100%` and clip the last glyph of "@ AMD" on mobile. Removing it reintroduces the clipping bug.
